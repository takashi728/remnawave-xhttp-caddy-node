...
if [ -f /etc/os-release ]; then
  ...
  case "${ID:-}" in
    ...
    *)
      echo "This installer is intended for Arch-based systems. Detected: ${ID:-unknown}" >&2
      exit 1
      ;;
  esac
fi
...
echo "[1/7] Installing Arch Linux dependencies"
pacman -Sy --needed --noconfirm ca-certificates curl docker docker-compose openssl cronie
...
if ! docker compose version >/dev/null 2>&1; then
  echo "Docker Compose is unavailable after package install." >&2
  exit 1
fi
...
if command -v ufw >/dev/null 2>&1; then
  ...
  echo "UFW is not installed. Make sure ports 22, 80, 443, and 2222/tcp are allowed by your firewall."
  if [ -n "$EXTRA_PORTS" ]; then
    echo "Also allow these additional inbound TCP ports: $EXTRA_PORTS"
  fi
fi
